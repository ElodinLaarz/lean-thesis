...
/-!
# The discriminant identity `(τ − τ̄)² = d`

**Thesis.** §3.2, the discriminant of the order: for an honest discriminant
`d` (i.e. `d ≡ 0, 1 (mod 4)`), `(τ − τ̄)² = d`.

**This file proves:**

* `tau_sub_tauConj`               — `τ − τ̄ = 2τ − d`
* `tau_sub_tauConj_sq`            — the general identity, valid for *all* `d`
* `tau_sub_tauConj_sq_of_valid_disc` — specialises to `(τ − τ̄)² = d` when
  `d ≡ 0 ∨ 1 (mod 4)` (the case used in the thesis)

**Divergence from thesis.** The thesis only ever needs `(τ − τ̄)² = d` because
...
remainder term: `(τ − τ̄)² = (d² − 4⌊(d²−d)/4⌋) • 1`, which collapses to `d`
