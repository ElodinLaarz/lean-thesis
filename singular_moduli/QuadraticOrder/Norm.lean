...
* `tauConj`             — the Galois conjugate `τ̄ = d − τ` (other root of `poly d`)
* `tau_add_tauConj` / `tau_mul_tauConj` — the Vieta relations
* `normForm_eq_mul_conj`— `(a + bτ)(a + bτ̄) = N(a, b)`
...
Galois conjugate `τ̄` and the Vieta relations `τ + τ̄ = d`, `τ·τ̄ = (d²−d)/4`,
which exhibit `N` as an honest multiplicative norm `α ↦ α·ᾱ`. The end formula
