...
* $K = \mathbb{Q}(\sqrt{d}) = \mathbb{Q}(\sqrt{D})$ is the imaginary quadratic field of
  discriminant $D$. Set
  $$\tau_d := \frac{d + \sqrt{d}}{2}, \qquad \mathcal{O}_d := \mathbb{Z}[\tau_d] = \mathbb{Z} \oplus \mathbb{Z}\,\tau_d,$$
  the order of discriminant $d$ in $K$; $\mathcal{O}_K = \mathcal{O}_D = \mathbb{Z}[\tau_D]$ is
  the maximal order. Since $\sqrt{d} = f\sqrt{D}$ we have
...
* For a nonzero ideal $\mathfrak{a} \subseteq \mathcal{O}$ ($\mathcal{O}$ one of
  ...
  $\mathbb{Z}\text{-module}$ $\mathcal{O}$.
* *Conjugation* $x \mapsto \bar x$ is the nontrivial field automorphism of $K$. It is a
  ring automorphism stabilizing $\mathcal{O}_d$ (indeed $\bar\tau_d = d - \tau_d \in \mathcal{O}_d$)
  and $\mathcal{O}_K$; hence for any ideal $\mathfrak{a}$, the conjugate
  $\bar{\mathfrak{a}}$ is an ideal of the same index.
* A nonzero ideal $\mathfrak{a} \subseteq \mathcal{O}_d$ is *invertible* if there is a
  fractional $\mathcal{O}_d\text{-ideal}$ $\mathfrak{b}$ (a nonzero finitely generated
  $\mathcal{O}_d\text{-submodule}$ of $K$) with $\mathfrak{a}\mathfrak{b} = \mathcal{O}_d$. In the
  Dedekind domain $\mathcal{O}_K$ every nonzero ideal is invertible.
* Counting functions, for $n \geq 1$:
  $$r_d(n) := \#\{\mathfrak{a} \subseteq \mathcal{O}_d \text{ ideal} : [\mathcal{O}_d : \mathfrak{a}] = n\}, \qquad r_d^{\times}(n) := \#\{\mathfrak{a} \subseteq \mathcal{O}_d \text{ invertible ideal} : [\mathcal{O}_d : \mathfrak{a}] = n\},$$
...
* $v_p$ is the $p\text{-adic}$ valuation; $w := v_p(f)$. Following the thesis's convention (with
  the ERRATA E6.8 clarification), for $p \mid f$ we write
  $$v := v_p(d/4) := \begin{cases} v_p(d) & p \text{ odd},\\ v_2(d) - 2 & p = 2 \end{cases}$$
  (when $2 \mid f$ one has $v_2(d) = v_2(D) + 2w \geq 2$, so $4 \mid d$ and this is a
  nonnegative integer — cf. notation.md Remark 5, which corrects the stronger claim
  $16 \mid d$; in fact $v = 2w + v_p(D)$ for odd $p$ and $v = 2w - 2 + v_2(D)$ for $p = 2$).
...
* Splitting trichotomy: a rational prime $p$ is *ramified* in $\mathcal{O}_K$ iff
  ...
  split iff $D \equiv 1 \pmod 8$, inert iff $D \equiv 5 \pmod 8$). Throughout,
...
> $$\{\mathfrak{a} \subseteq \mathcal{O}_d : [\mathcal{O}_d : \mathfrak{a}] = n\} \;\longleftrightarrow\; \prod_{p \mid n} \{\mathfrak{b} \subseteq \mathcal{O}_d : [\mathcal{O}_d : \mathfrak{b}] = p^{v_p(n)}\},$$
> under which $\mathfrak{a}$ is invertible if and only if every component is invertible.
> Consequently
> $$r_d(n) = \prod_{p \mid n} r_d\!\left(p^{v_p(n)}\right) \qquad\text{and}\qquad r_d^{\times}(n) = \prod_{p \mid n} r_d^{\times}\!\left(p^{v_p(n)}\right).$$
...
> $$\mathfrak{a} \longmapsto \mathfrak{a}\mathcal{O}_K, \qquad \mathfrak{A} \longmapsto \mathfrak{A} \cap \mathcal{O}_d$$
> are mutually inverse, index-preserving bijections
> $$\{\mathfrak{a} \subseteq \mathcal{O}_d : [\mathcal{O}_d : \mathfrak{a}] = n\} \;\longleftrightarrow\; \{\mathfrak{A} \subseteq \mathcal{O}_K : [\mathcal{O}_K : \mathfrak{A}] = n\}.$$
...
> $$t_p(m) = u_p(m) = \begin{cases} m + 1 & \text{$p$ split in } \mathcal{O}_K,\\ \varepsilon(m) & \text{$p$ inert in } \mathcal{O}_K,\\ 1 & p \mid D. \end{cases}$$
>
> *If* $p \mid f$, with $w = v_p(f) \geq 1$ and $v = v_p(d/4)$, the **corrected**
> Theorem 3.1.2 (ERRATA E4) gives the total local factor
> $$t_p(m) = \begin{cases} \dfrac{p^{\lfloor m/2\rfloor + 1} - 1}{p - 1} & m \leq v,\\[2ex] \dfrac{p^{w+1} - 1}{p - 1} & m > v, \; p \mid D,\\[2ex] \dfrac{p^{w+\varepsilon(m)} - 1}{p - 1} & m > v, \; p \text{ inert},\\[2ex] \dfrac{p^{w+\varepsilon(m)} - 1}{p - 1} + p^w\,(m + 1 - 2w - \varepsilon(m)) & m > v, \; p \text{ split}, \end{cases}$$
> and the **corrected** Corollary 3.1.3 (ERRATA E5) gives the invertible local factor
> $$u_p(m) = \begin{cases} \varepsilon(m)\, p^{m/2} & m < v, \text{ or } \bigl(m = v \text{ and } v = 2w-2\bigr),\\ p^w & \bigl(m > v, \text{ or } m = v \geq 2w\bigr), \; p \mid D,\\ \varepsilon(m)\,\bigl(p^w + p^{w-1}\bigr) & \bigl(m > v, \text{ or } m = v \geq 2w\bigr), \; p \text{ inert},\\ \bigl(p^w - p^{w-1}\bigr)(m + 1 - 2w) & \bigl(m > v, \text{ or } m = v \geq 2w\bigr), \; p \text{ split} \end{cases}$$
...
> $$r_d(n) = \prod_{p \mid n} t_p\bigl(v_p(n)\bigr), \qquad r_d^{\times}(n) = \prod_{p \mid n} u_p\bigl(v_p(n)\bigr).$$
...
**Fact 0.4.** *If $\mathfrak{b}, \mathfrak{c}$ have finite indices $N_\mathfrak{b}, N_\mathfrak{c}$,
then $[\mathcal{O} : \mathfrak{b}\mathfrak{c}]$ is finite and divides
...
$[\mathcal{O} : N_\mathfrak{b}\mathfrak{c}] = [\mathcal{O} : N_\mathfrak{b}\mathcal{O}]\,[N_\mathfrak{b}\mathcal{O} : N_\mathfrak{b}\mathfrak{c}] = N_\mathfrak{b}^2\, N_\mathfrak{c}$
...
$x = xb + xc \in \mathfrak{c}\mathfrak{b} + \mathfrak{b}\mathfrak{c} = \mathfrak{b}\mathfrak{c}$.
...
$\mathcal{O}\text{-module}$ of cardinality $n$.

**Step 1.1 (primary decomposition of $M$ is $\mathcal{O}\text{-stable}$).** As a finite abelian
group, $M = \bigoplus_{p \mid n} M_p$, where
$M_p := \{x \in M : p^k x = 0 \text{ for some } k\}$ is the $p\text{-primary}$ (Sylow)
component, of order $p^{v_p(n)}$ (structure theorem for finite abelian groups). Each
$M_p$ is an $\mathcal{O}\text{-submodule}$: multiplication by any $y \in \mathcal{O}$ is an
...
$\mathcal{O}\text{-modules}$ and the projections $\pi_p : M \to M_p$ are $\mathcal{O}\text{-linear}$.
...
$$\bigcap_{p \mid n} \mathfrak{a}_p = \ker\Bigl(\mathcal{O} \to \bigoplus_{p \mid n} M_p = M\Bigr) = \mathfrak{a}.$$
...
whence $[\mathcal{O} : \mathfrak{b}] = \prod_p p^{v_p(n)} = n$. The $p\text{-primary}$ component
of the right-hand side is exactly the factor $\mathcal{O}/\mathfrak{b}_p$ (a $p\text{-group}$,
...
**Case: every $\mathfrak{a}_p$ is invertible.** Say
...
**Case: $\mathfrak{a}$ is invertible.** Say $\mathfrak{a}\mathfrak{c} = \mathcal{O}$; then for
each $p$,
$$\mathfrak{a}_p \cdot \Bigl(\mathfrak{c} \prod_{q \neq p} \mathfrak{a}_q\Bigr) = \mathfrak{c}\,\mathfrak{a} = \mathcal{O},$$
...
2. *the inclusion $\mathcal{O}_d \hookrightarrow \mathcal{O}_K$ induces a ring
   isomorphism $\mathcal{O}_d/N\mathcal{O}_d \xrightarrow{\;\sim\;} \mathcal{O}_K/N\mathcal{O}_K$.*
...
(2) The composite $\mathcal{O}_d \hookrightarrow \mathcal{O}_K \to \mathcal{O}_K/N\mathcal{O}_K$
...
$N^2$, as quotients of free rank-$2$ $\mathbb{Z}\text{-modules}$ by $N\text{-multiples}$.)
$\blacksquare$
...
(3) Under the isomorphism $\varphi : \mathcal{O}_d/N\mathcal{O}_d \to \mathcal{O}_K/N\mathcal{O}_K$
of Lemma 2.1(2), the image of the ideal $\mathfrak{c}/N\mathcal{O}_d$ is
$(\mathfrak{c} + N\mathcal{O}_K)/N\mathcal{O}_K = \mathfrak{c}\mathcal{O}_K/N\mathcal{O}_K$
by (1). Since $\varphi$ is an isomorphism of rings,
$$[\mathcal{O}_K : \mathfrak{c}\mathcal{O}_K] = \bigl[\mathcal{O}_K/N\mathcal{O}_K : \mathfrak{c}\mathcal{O}_K/N\mathcal{O}_K\bigr] = \bigl[\mathcal{O}_d/N\mathcal{O}_d : \mathfrak{c}/N\mathcal{O}_d\bigr] = N. \qquad \blacksquare$$
...
prime ideals, where it is checked case by case: $\mathfrak{p}\bar{\mathfrak{p}} = p\mathcal{O}_K$
for $p$ split, $\mathfrak{p}\bar{\mathfrak{p}} = \mathfrak{p}^2 = p^2\mathcal{O}_K$ for $p$
inert, $\mathfrak{p}\bar{\mathfrak{p}} = \mathfrak{p}^2 = p\mathcal{O}_K$ for $p$ ramified). Now
consider
...
$$\mathfrak{a}\bar{\mathfrak{a}} = \bigl(\mathfrak{a}\bar{\mathfrak{a}}\,\mathcal{O}_K\bigr) \cap \mathcal{O}_d = \bigl(\mathfrak{A}\bar{\mathfrak{A}}\bigr) \cap \mathcal{O}_d = n\mathcal{O}_K \cap \mathcal{O}_d = n\mathcal{O}_d,$$
using $(\mathfrak{a}\bar{\mathfrak{a}})\mathcal{O}_K = (\mathfrak{a}\mathcal{O}_K)(\bar{\mathfrak{a}}\mathcal{O}_K) = \mathfrak{A}\bar{\mathfrak{A}}$
(extension is multiplicative and commutes with
...
By Proposition 1,
$$r_d(n) = \prod_{p \mid n} r_d\!\left(p^{m_p}\right), \qquad r_d^{\times}(n) = \prod_{p \mid n} r_d^{\times}\!\left(p^{m_p}\right).$$
...
E4, the $p\text{-uniform}$ version) evaluates $r_d(p^{m})$ to $t_p(m)$, and the corrected
...
in two of its five $2\text{-adic}$ branches (ERRATA E1, E2, root cause E3; corrected $p\text{-uniform}$
...
non-invertible ideals of a non-maximal order,
$[\mathcal{O} : \mathfrak{a}\mathfrak{b}] = [\mathcal{O} : \mathfrak{a}][\mathcal{O} : \mathfrak{b}]$
**fails**. Example: $d = -12$ ($D = -3$, $f = 2$), $\mathcal{O}_{-12} = \mathbb{Z}[\sqrt{-3}]$, and
...
(as a lattice, $\mathfrak{p}^2 = \{x + y\sqrt{-3} : x \equiv y \equiv 0 \bmod 2,\; x \equiv y \bmod 4\}$).
This is exactly why Proposition 1 is proved via comaximal
...
$\mathbb{Z}\text{-basis}$. It is also precisely where Mathlib's conductor machinery applies: the
...
`AddCommGroup`-structure-theorem material; the $\mathcal{O}\text{-stability}$ argument is one
...
$d = -36$; $u_3(3) = 3$ for $d = -27$. Composite check: $r_{-12}(6) = t_2(1)\,t_3(1) = 1 \cdot 1 = 1$
(the ideal $\mathfrak{p} \cdot \sqrt{-3}\,\mathcal{O}$). These belong in
...
* `ERRATA.md` (repo root): E1–E4 (corrected Theorem 3.1.2, $p\text{-uniform}$), E5 (corrected
...
* K. Lauter, B. Viray, *On singular moduli for arbitrary discriminants* ([LV15b]):
  Thm 1.5 and Conj 1.7 (= thesis Thm 2.4.2, Conj 2.4.3) — the formulas against which the
  falsification loop tests candidate reformulations of $A(N)$.
